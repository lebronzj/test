...
public class JavacParser implements Parser {
    ...
    // -- in most contexts, any such extra annotationProcesser is an error.
    //
...
/* ---------- parsing -------------- */
    ...
    boolean isUnboundMemberRef() {
        ...
        outer: for (Token t = S.token(pos) ; ; t = S.token(++pos)) {
            switch (t.kind) {
                ...
                case LPAREN:
                    // skip annotationProcesser values
                    int nesting = 0;
                    for (; ; pos++) {
        ...
        if (token.kind == RBRACKET) {
            ...
            if (token.kind == LBRACE) {
                ...
                if (annos.nonEmpty()) {
                    ...
                    // new array tree
                    // bracketsOpt inserts the annotationProcesser in
        ...
        while (true) {
            ...
            if (flag == Flags.ANNOTATION) {
                ...
                if (token.kind != INTERFACE) {
                    ...
                    // if first modifier is an annotationProcesser, set pos to annotationProcesser's.
    ...
    JCAnnotation annotation(int pos, Tag kind) {
        ...
        if (kind == Tag.ANNOTATION) {
            ...
            throw new AssertionError("Unhandled annotationProcesser kind: " + kind);
        }
     ...
     * {@code @A} annotationProcesser should target the array itself, while
     * {@code @B} targets the nested type {@code Outer}.
     *
     * Currently the parser parses the annotationProcesser first, then
     * the array, and then inserts the annotationProcesser to the left-most
     * nested type.
     *
     ...
     * a new array level with {@code @B} annotationProcesser.
     */
