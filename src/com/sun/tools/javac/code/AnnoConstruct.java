...
public abstract class AnnoConstruct implements AnnotatedConstruct {
    ...
    public <A extends Annotation> A[] getAnnotationsByType(Class<A> annoType) {
        ...
        if (!annoType.isAnnotation())
            throw new IllegalArgumentException("Not an annotationProcesser type: "
                                               + annoType);
    ...
    public <A extends Annotation> A getAnnotation(Class<A> annoType) {
        ...
        if (!annoType.isAnnotation())
            throw new IllegalArgumentException("Not an annotationProcesser type: " + annoType);
    ...
    private static Class<? extends Annotation> getContainer(Class<? extends Annotation> annoType) {
        // Since we can not refer to java.lang.annotationProcesser.Repeatable until we are
        // bootstrapping with java 8 we need to get the Repeatable annotationProcesser using
        ...
        if (REPEATABLE_CLASS != null &&
            ...
            if (repeatable != null) {
                try {
                    ...
                    // indicating the containing annotationProcesser type
                    @SuppressWarnings("unchecked")
