 ...
 *************************************************************************/
    ...
    public Object attribLazyConstantValue(Env<AttrContext> env,
        ...
        try {
            // Use null as symbol to not attach the type annotationProcesser to any symbol.
    ...
    public void visitClassDef(JCClassDecl tree) {
        ...
        if ((env.info.scope.owner.kind & (VAR | MTH)) != 0) {
            ...
            // If this class declaration is part of a class level annotationProcesser,
        ...
        try {
            ...
            if ((owner.flags() & ANNOTATION) != 0 &&
                ...
                log.error(tree.params.head.pos(),
                          "intf.annotationProcesser.members.cant.have.params");
            ...
            // annotationProcesser method checks
            if ((owner.flags() & ANNOTATION) != 0) {
                // annotationProcesser method cannot have throws clause
                if (tree.thrown.nonEmpty()) {
                    log.error(tree.thrown.head.pos(),
                            "throws.not.allowed.in.intf.annotationProcesser");
                }
                // annotationProcesser method cannot declare type-parameters
                if (tree.typarams.nonEmpty()) {
                    log.error(tree.typarams.head.pos(),
                            "intf.annotationProcesser.members.cant.have.type.params");
                }
                // validate annotationProcesser method's return type (could be an annotationProcesser type)
                chk.validateAnnotationType(tree.restype);
                // ensure that annotationProcesser method does not clash with members of Object/Annotation
            ...
            if (tree.body == null) {
                ...
                if (tree.defaultValue != null) {
                    if ((owner.flags() & ANNOTATION) == 0)
                        log.error(tree.pos(),
                                  "default.allowed.in.intf.annotationProcesser.member");
                }
...
//                             Resolve.isStatic(env));//DEBUG
    ...
    private static List<Attribute.TypeCompound> fromAnnotations(List<JCAnnotation> annotations) {
        ...
        for (JCAnnotation anno : annotations) {
            ...
            // when attempting to attach an annotationProcesser that isn't
            // defined.
        }
        return buf.toList();
    }
    ...
    private void attribClassBody(Env<AttrContext> env, ClassSymbol c) {
        ...
        if ((c.flags() & ANNOTATION) != 0) {
            if (tree.implementing.nonEmpty())
                log.error(tree.implementing.head.pos(),
                          "cant.extend.intf.annotationProcesser");
            if (tree.typarams.nonEmpty())
                log.error(tree.typarams.head.pos(),
                          "intf.annotationProcesser.cant.have.type.params");

            // If this annotationProcesser has a @Repeatable, validate
        ...
        // Check for cycles among annotationProcesser elements.
        ...
        private void checkForDeclarationAnnotations(List<? extends JCAnnotation> annotations,
            ...
            for (JCAnnotation ai : annotations) {
                if (!ai.type.isErroneous() &&
                    ...
                    log.error(ai.pos(), "annotationProcesser.type.not.applicable");
                }
            }
        }
    };
