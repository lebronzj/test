 ...
 **************************************************************************/
     ...
     *  enum, an annotationProcesser, Class, Class<?>, Class<? extends
    ...
    void validateAnnotationType(DiagnosticPosition pos, Type type) {
        ...
        log.error(pos, "invalid.annotationProcesser.member.type");
    }

    /**
     * "It is also a compile-time error if any method declared in an
     * annotationProcesser type has a signature that is override-equivalent to
     * that of any public or protected method declared in class Object
     * or in the interface annotationProcesser.Annotation."
     *
        ...
        for (Type sup = syms.annotationType; sup.hasTag(CLASS); sup = types.supertype(sup)) {
            ...
            for (Scope.Entry e = s.lookup(m.name); e.scope != null; e = e.next()) {
                if (e.sym.kind == MTH &&
                    ...
                    log.error(pos, "intf.annotationProcesser.member.clash", e.sym, sup);
            }
        }
    }
    ...
    /** Check an annotationProcesser of a symbol.
     */
    private void validateAnnotation(JCAnnotation a, Symbol s) {
        validateAnnotationTree(a);

        if (!annotationApplicable(a, s))
            log.error(a.pos(), "annotationProcesser.type.not.applicable");
    ...
    public void validateTypeAnnotation(JCAnnotation a, boolean isTypeParameter) {
        Assert.checkNonNull(a.type, "annotationProcesser tree hasn't been attributed yet: " + a);
        validateAnnotationTree(a);
     ...
     * annotationProcesser type symbol 's'. Report errors at position
     * 'pos'.
     *
     * @param s The (annotationProcesser)type declaration annotated with a @Repeatable
    ...
    private void validateValue(TypeSymbol container, TypeSymbol contained, DiagnosticPosition pos) {
        ...
        if (e.scope != null && e.sym.kind == MTH) {
            ...
            if (!(ret.hasTag(ARRAY) && types.isSameType(((ArrayType)ret).elemtype, contained.type))) {
                log.error(pos, "invalid.repeatable.annotationProcesser.value.return",
                        container, ret, types.makeArrayType(contained.type));
            }
        } else {
            log.error(pos, "invalid.repeatable.annotationProcesser.no.value", container);
        }
    }
    ...
    private void validateRetention(Symbol container, Symbol contained, DiagnosticPosition pos) {
        ...
        if (error ) {
            log.error(pos, "invalid.repeatable.annotationProcesser.retention",
    ...
    private void validateDocumented(Symbol container, Symbol contained, DiagnosticPosition pos) {
        if (contained.attribute(syms.documentedType.tsym) != null) {
            if (container.attribute(syms.documentedType.tsym) == null) {
                log.error(pos, "invalid.repeatable.annotationProcesser.not.documented", container, contained);
            }
        }
    }
    ...
    private void validateInherited(Symbol container, Symbol contained, DiagnosticPosition pos) {
        if (contained.attribute(syms.inheritedType.tsym) != null) {
            if (container.attribute(syms.inheritedType.tsym) == null) {
                log.error(pos, "invalid.repeatable.annotationProcesser.not.inherited", container, contained);
            }
        }
    }
    ...
    private void validateTarget(Symbol container, Symbol contained, DiagnosticPosition pos) {
        ...
        // (with respect to annotationProcesser target semantics) of the set of targets
        ...
        if (!isTargetSubsetOf(containerTargets, containedTargets)) {
            log.error(pos, "invalid.repeatable.annotationProcesser.incompatible.target", container, contained);
        }
    }
    ...
    private void validateDefault(Symbol container, DiagnosticPosition pos) {
        ...
        for(Symbol elm : scope.getElements()) {
            if (elm.name != names.value &&
                ...
                log.error(pos,
                          "invalid.repeatable.annotationProcesser.elem.nondefault",
                          container,
                          elm);
            }
        }
    ...
    /** Is the annotationProcesser applicable to types? */
    protected boolean isTypeAnnotation(JCAnnotation a, boolean isTypeParameter) {
        Compound atTarget =
            a.annotationType.type.tsym.attribute(syms.annotationTargetType.tsym);
        if (atTarget == null) {
            // An annotationProcesser without @Target is not a type annotationProcesser.
            return false;
        }
    ...
    /** Is the annotationProcesser applicable to the symbol? */
    ...
    /** Check an annotationProcesser value.
     *
     * @param a The annotationProcesser tree to check
     * @return true if this annotationProcesser tree is valid, otherwise false
     */
    ...
    private boolean validateAnnotation(JCAnnotation a) {
        ...
        // collect an inventory of the annotationProcesser elements
        ...
        for (JCTree arg : a.args) {
            ...
            if (!members.remove(m)) {
                isValid = false;
                log.error(assign.lhs.pos(), "duplicate.annotationProcesser.member.value",
                          m.name, a.type);
            }
        }
        ...
        if (missingDefaults.nonEmpty()) {
            ...
            String key = (missingDefaults.size() > 1)
                    ? "annotationProcesser.missing.default.value.1"
                    : "annotationProcesser.missing.default.value";
            log.error(a.pos(), key, a.type, missingDefaults);
        }

        // special case: java.lang.annotationProcesser.Target must not have
        ...
        for (JCTree elem : na.elems) {
            if (!targets.add(TreeInfo.symbol(elem))) {
                isValid = false;
                log.error(elem.pos(), "repeated.annotationProcesser.target");
            }
        }
        return isValid;
    }
    ...
    void checkDeprecatedAnnotation(DiagnosticPosition pos, Symbol s) {
        if (allowAnnotations &&
            ...
            log.warning(LintCategory.DEP_ANN,
                    pos, "missing.deprecated.annotationProcesser");
        }
    }
...
/* *************************************************************************
 * Check for recursive annotationProcesser elements.
 **************************************************************************/

    /** Check for cycles in the graph of annotationProcesser elements.
     */
    ...
    void checkNonCyclicElementsInternal(DiagnosticPosition pos, TypeSymbol tsym) {
        ...
        if ((tsym.flags_field & LOCKED) != 0) {
            log.error(pos, "cyclic.annotationProcesser.element");
            return;
        }
        try {
