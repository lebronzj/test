 ...
 *********************************************************************/
    ...
    public class AnnotateRepeatedContext<T extends Attribute.Compound> {
         ...
         *
         * @param a the Worker to enqueue for repeating annotationProcesser annotating
         */
...
/* ********************************************************************
 * Compute an attribute from its annotationProcesser.
 *********************************************************************/

    /** Process a single compound annotationProcesser, returning its
    ...
    Attribute.Compound enterAnnotation(JCAnnotation a,
        ...
        // The annotationProcesser might have had its type attributed (but not checked)
        ...
        if ((a.type.tsym.flags() & Flags.ANNOTATION) == 0) {
            log.error(a.annotationType.pos(),
                      "not.annotationProcesser.type", a.type.toString());
        ...
        for (List<JCExpression> tl = args; tl.nonEmpty(); tl = tl.tail) {
            ...
            if (!t.hasTag(ASSIGN)) {
                log.error(t.pos(), "annotationProcesser.value.must.be.name.value");
                continue;
            }
            ...
            if (method.owner != a.type.tsym)
                log.error(left.pos(), "no.annotationProcesser.member", left.name, a.type);
    ...
    Attribute enterAttributeValue(Type expected,
        ...
        if (expected.hasTag(ARRAY)) {
            ...
            if (na.elemtype != null) {
                log.error(na.elemtype.pos(), "new.not.allowed.in.annotationProcesser");
            }
        ...
        if (tree.hasTag(NEWARRAY)) { //error recovery
            if (!expected.isErroneous())
                log.error(tree.pos(), "annotationProcesser.value.not.allowable.type");
            JCNewArray na = (JCNewArray)tree;
            if (na.elemtype != null) {
                log.error(na.elemtype.pos(), "new.not.allowed.in.annotationProcesser");
            }
        ...
        if ((expected.tsym.flags() & Flags.ANNOTATION) != 0) {
            if (tree.hasTag(ANNOTATION)) {
                ...
                log.error(tree.pos(), "annotationProcesser.value.must.be.annotationProcesser");
                expected = syms.errType;
            }
        }
        if (tree.hasTag(ANNOTATION)) { //error recovery
            if (!expected.isErroneous())
                log.error(tree.pos(), "annotationProcesser.not.valid.for.type", expected);
        ...
        if (expected.tsym == syms.classType.tsym) {
            ...
            // at the tree level
            if (TreeInfo.name(tree) != names._class) {
                log.error(tree.pos(), "annotationProcesser.value.must.be.class.literal");
                return new Attribute.Error(syms.errType);
            }
        ...
        if (expected.hasTag(CLASS) &&
            ...
            if (sym == null ||
                ...
                log.error(tree.pos(), "enum.annotationProcesser.must.be.enum.constant");
                return new Attribute.Error(result.getOriginalType());
            }
     ...
     * synthesized container annotationProcesser or null IFF all repeating
     * annotationProcesser are invalid.  This method reports errors/warnings.
     */
    private <T extends Attribute.Compound> T processRepeatedAnnotations(List<T> annotations,
             ...
             al = al.tail)
        {
            ...
            // There must be more than a single anno in the annotationProcesser list
            ...
            // annos of the same annotationProcesser type, the types should
        ...
        if (!repeated.isEmpty()) {
            ...
            if (ctx.isTypeCompound) {
                ...
                */
                ...
                // TODO: annotationProcesser applicability checks from below?
                ...
                if (!chk.annotationApplicable(annoTree, on))
                    log.error(annoTree.pos(), "invalid.repeatable.annotationProcesser.incompatible.target", targetContainerType, origAnnoType);

                if (!chk.validateAnnotationDeferErrors(annoTree))
                    log.error(annoTree.pos(), "duplicate.annotationProcesser.invalid.repeated", origAnnoType);
    ...
    /** Fetches the actual Type that should be the containing annotationProcesser. */
            ...
            boolean reportError)
    {
        ...
        // Fetch the Repeatable annotationProcesser from the current
        // annotationProcesser's declaration, or null if it has none
        Attribute.Compound ca = origAnnoDecl.attribute(syms.repeatableType.tsym);
        if (ca == null) { // has no Repeatable annotationProcesser
            if (reportError)
                log.error(pos, "duplicate.annotationProcesser.missing.container", origAnnoType, syms.repeatableType);
            return null;
        }
    ...
    /** Extract the actual Type to be used for a containing annotationProcesser. */
    private Type extractContainingType(Attribute.Compound ca,
            DiagnosticPosition pos,
            TypeSymbol annoDecl)
    {
        // The next three checks check that the Repeatable annotationProcesser
        // on the declaration of the annotationProcesser type that is repeating is
        // valid.

        // Repeatable must have at least one element
        if (ca.values.isEmpty()) {
            log.error(pos, "invalid.repeatable.annotationProcesser", annoDecl);
            return null;
        }
    ...
    private MethodSymbol validateContainer(Type targetContainerType,
        ...
        if (error) {
            log.error(pos,
                      "invalid.repeatable.annotationProcesser.multiple.values",
                      targetContainerType,
                      nr_value_elems);
            return null;
        } else if (nr_value_elems == 0) {
            log.error(pos,
                      "invalid.repeatable.annotationProcesser.no.value",
                      targetContainerType);
            return null;
        }
        ...
        if (containerValueSymbol.kind != Kinds.MTH) {
            log.error(pos,
                      "invalid.repeatable.annotationProcesser.invalid.value",
                      targetContainerType);
            fatalError = true;
        }
        ...
        if (!(types.isArray(valueRetType) &&
            ...
            log.error(pos,
                      "invalid.repeatable.annotationProcesser.value.return",
                      targetContainerType,
                      valueRetType,
                      expectedType);
            fatalError = true;
        ...
        // The conditions for a valid containing annotationProcesser are made
